  ...
  if (wifiStateManager.getCurrentState() != WiFiState_e::WiFiState_Connected) {
    ...
    if (!MDNS.begin(DEFAULT_HOSTNAME)) {
      ...
      if (n == 0) {
        ...
        setMQTTBroker(mqttServer.toString().c_str(), mqttPort);
...
void mDNSDiscovery::setMQTTBroker(const std::string& broker, int port) {
  brokerConfig.broker.assign(broker);
  brokerConfig.port = port;
}

const mDNSDiscovery::MQTTBroker* mDNSDiscovery::getMQTTBroker() {
  const mDNSDiscovery::MQTTBroker* broker = &brokerConfig;
  return broker;
}
